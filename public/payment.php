    ...
    <div class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-lg">
        ...
        <form method="POST" class="mb-6">
            <div class="mb-4">
                ...
                <input type="text" id="name" name="name" class="w-full px-4 py-2  rounded-lg " value="<?php echo $_SESSION['user_name']; ?>" readonly>
            </div>
            <div class="mb-4">
                <label for="total_bill" class="block text-gray-700">Total Bill (PHP)</label>
                <input type="text" id="total_bill" name="total_bill" class="w-full px-4 py-2  rounded-lg " value="<?php echo number_format($_SESSION['totalBill'], 2); ?>" readonly>
            </div>
            <div class="mb-4">
                <label for="amount_paid" class="block text-gray-700">Amount to Pay (PHP)</label>
                <input type="number" step="0.01" id="amount_paid" name="amount_paid" class="w-full px-4 py-2  rounded-lg " required>
            </div>
            <div class="mb-4">
                <label for="credit_card" class="block text-gray-700">Credit Card Number</label>
                <input type="text" id="credit_card" name="credit_card" class="w-full px-4 py-2  rounded-lg" required>
            </div>
            <button type="submit" name="pay" class="w-full bg text-white px-4 py-2 rounded-lg ">Pay</button>
        </form>
    </div>
</body>
</html>
